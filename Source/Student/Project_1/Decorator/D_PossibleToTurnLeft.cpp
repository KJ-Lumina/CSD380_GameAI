#include <pch.h>
#include "D_Delay.h"
#include "D_PossibleToTurnLeft.h"
#include "../GlobalInfo.h"

D_PossibleToTurnLeft::D_PossibleToTurnLeft()
{}

void D_PossibleToTurnLeft::on_enter()
{
    BehaviorNode::on_enter();
}

void D_PossibleToTurnLeft::on_update(float dt)
{
	...
	//if(turnDirection == MovementDirection::LEFT)
	//{
	//	child->tick(dt);
    ...
    for(JunctionOrientationPassasge& passages : junction.orientationPassages)
    {
		...
		if (cosTheta > 1.0f - FLT_EPSILON && passages.turnDirection == MovementDirection::LEFT)
	    {
            child->tick(dt);
